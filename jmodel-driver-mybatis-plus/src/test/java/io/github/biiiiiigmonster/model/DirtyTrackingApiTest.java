 ...
 * <p>
 * 覆盖 Model 中所有 dirty-tracking 相关 API 方法：
 * <ul>
 *   <li>追踪状态测试（UNTRACKED / TRACKING）</li>
 *   <li>isDirty() 系列方法</li>
 *   <li>getDirty() 系列方法</li>
 *   <li>getOriginal() 系列方法</li>
 *   <li>wasChanged() / getChanges() 方法</li>
 *   <li>syncOriginal() 系列方法</li>
 *   <li>关系字段排除测试</li>
 *   <li>修改后恢复原值测试</li>
 *   <li>多次修改测试</li>
 * </ul>
...
public class DirtyTrackingApiTest extends BaseTest {
    ...
    /**
     * 未调用 syncOriginal 时，isDirty 应返回 false
     */
    @Test
    ...
    /**
     * syncOriginal 后 setter 调用，isDirty 应返回 true
     */
    @Test
    ...
    /**
     * 未调用 syncOriginal 时，setter 调用不触发追踪
     */
    @Test
    ...
    /**
     * syncOriginal 后未修改，isDirty 应为 false
     */
    @Test
    ...
    /**
     * isDirty(String... fields) 多字段任一匹配返回 true
     */
    @Test
    ...
    /**
     * isDirty(SerializableFunction) Lambda 版本
     */
    @Test
    ...
    /**
     * getOriginal(field) 返回 syncOriginal 时的值
     */
    @Test
    ...
    /**
     * getOriginal(SerializableFunction) Lambda 版本
     */
    @Test
    ...
    // ==================== 4. wasChanged / getChanges 测试 ====================

    /**
     * save 后 wasChanged() 返回 true
     */
    @Test
    ...
    /**
     * save 前 getChanges() 返回空 Map（savedChanges 默认为空）
     */
    @Test
    ...
    /**
     * syncOriginal(String... fields) 部分字段同步
     */
    @Test
    ...
    /**
     * syncOriginal(fields) 对未追踪实体会先执行全量同步
     */
    @Test
    public void testSyncOriginal_partialFields_onUntrackedEntity() {
        ...
        // 未追踪状态下调用 syncOriginal(fields) 应先全量同步
        user.syncOriginal("name");
    ...
    /**
     * 关系字段（@HasOne, @HasMany, @BelongsToMany, @MorphOne 等）不被追踪
     */
    @Test
    ...
    /**
     * 完整流程：加载 → syncOriginal → 修改 → save → 检查 changes
     */
    @Test
